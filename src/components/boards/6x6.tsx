...
import { emptyBoard, flattenAndCopy, is6x6BoardValid, is6x6CellValid, solve6x6Sudoku } from "../../utils";
...
export function Board6x6({ setDimension }: Props) {
   const n = 6;
   ...
   function handleSolve() {
      if (!is6x6BoardValid(board)) {
         window.alert("Invalid board! Duplicates found");
         return;
      }

      const boardCopy = board.map((row) => [...row]);
      const solved = solve6x6Sudoku(boardCopy);
      ...
      <div>
         ...
         <div className="my-8 flex flex-col items-center space-y-4">
            <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${n}, minmax(0, 1fr))`, width: "fit-content" }}>
               {board.map((row, r) =>
                  row.map((cell, c) => {
                     const valid = is6x6CellValid({ board, col: c, num: cell, row: r });
                     const isSubGrid = (Math.floor(r / 2) + Math.floor(c / 3)) % 2 === 0;
